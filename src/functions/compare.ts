...
const compareLength = (input: object | Array<object>, expected: string): boolean => {
	...
	/**
	 * Checks if value is higher or lower in order
	 * @param {string[]} items - Array with drivers/kexts
	 * @returns {boolean} - Boolean if input is NOT in order
	 */
	compareOrder = (array: string[], rule: string, req: string): boolean => {
		if (!array || !rule || !req) return false;
		const check = req.charAt(0);
		const key = req.slice(1);
		const ruleKey = rule.split(": ")[1];

		const index = array.findIndex(item => item.match(ruleKey));
		const reqIndex = array.findIndex(item => item.match(key));
		//console.log(`${ruleKey}: ${index}, ${key}: ${reqIndex}`);
		switch (check) {
			case ">":
...
export { compareLength, compareOrder, compareValue };
